...
router = APIRouter(prefix="/api/events", tags=["Version History"])
@router.get("/{id}/history/{version_id}", response_model=EventVersionSchema)
async def get_event_version(id: int, version_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    return get_event_version_service(id, version_id, db, current_user)


# Rollback to a specific version of an event
@router.post("/{id}/rollback/{version_id}", response_model=EventVersionSchema)
async def rollback_event(id: int, version_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    return rollback_event_service(id, version_id, db, current_user)